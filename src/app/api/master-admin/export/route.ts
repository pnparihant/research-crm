...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // writeFile supports password encryption; writeBuffer does not
    tmpPath = join(tmpdir(), `arihant_export_${Date.now()}.xlsx`);
   await (wb.xlsx as any).writeFile(tmpPath, { password: password.trim() });
    ...
    if (tmpPath) {
      unlink(tmpPath).catch((e) =>
        console.warn("[master-admin/export] Failed to delete tmp file:", e.message)
      );
    }
  }
}