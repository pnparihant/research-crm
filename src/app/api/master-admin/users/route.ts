...
export async function POST(req: NextRequest) {
  ...
  const deny = requireMasterAdmin(token, "POST");
  if (deny) return deny;
  ...
  console.log(`[master-admin/users] POST — created user email=${email.toLowerCase()} by master_admin=${token!.email}`);
...
export async function PUT(req: NextRequest) {
  ...
  const token = await getToken({ req });
  const deny = requireMasterAdmin(token, "PUT");
  if (deny) return deny;
  ...
  console.log(`[master-admin/users] PUT — updated user id=${id} by master_admin=${token!.email}`);
...
export async function DELETE(req: NextRequest) {
  ...
  const token = await getToken({ req });
  const deny = requireMasterAdmin(token, "DELETE");
  if (deny) return deny;
  if (!id) {
  ...
  console.log(`[master-admin/users] DELETE — deleted user email=${user.email} by master_admin=${token!.email}`);
